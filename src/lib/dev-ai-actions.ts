...
- **RL Coach Module**: RL coaches (\`rl_coach\` role) manage morning workouts **scoped to their own house** (via \`profiles.house_id\`). QR scanning for workout attendance (\`workout_attendance\` table) with present/late options. Finalize sessions to auto-flag absent students. Notifications are sent only when students are **late** (not absent) — stored in \`workout_notifications\`. RL coaches also scan **dinner** meal attendance for their house students.
- **Meal Attendance**: RL coaches handle **dinner** scanning for their house. Moderators handle **lunch** scanning. All via QR codes into \`meal_attendance\` table.
