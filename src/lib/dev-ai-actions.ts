...
- **RL Coach Module**: RL coaches (\`rl_coach\` role) manage morning workouts **scoped to their own house** (via \`profiles.house_id\`). QR scanning for workout attendance (\`workout_attendance\` table) with present/late options. Finalize sessions to auto-flag absent students. Notifications are sent only when students are **late** (not absent) — stored in \`workout_notifications\`. Workout attendance restricted to specific authorized users (praveen.rana, reverien.kiruzi, peter.otema, mauritz.viljoen, emma.doellefeld).
- **Meal Attendance**: RL coaches handle **house-based dinner** scanning (must select target house before scanning — can choose own house or any other). \`meal_attendance.house_id\` tracks which house the dinner scan was for. Moderators handle **lunch** scanning. Mentor teachers also scan lunch for their mentees via Teacher Dashboard. All via QR codes into \`meal_attendance\` table.
...
- **Class Selection**: Students choose their class (7A, 7B, 7C, 7D, 7E, 8A, 8B, 8C, 8D, 8E, 8F) on first login via \`ClassSelectionCard\`. Stored in \`profiles.student_class\`. One-time selection — only admins can change via User Management.
- **Mentor System**: Students select a mentor teacher from a predefined list via \`MentorSelectionCard\`. Stored in \`profiles.mentor_id\` (uuid referencing the teacher's profile). Mentor teachers gain lunch attendance scanning capability on their Teacher Dashboard. Admins can also assign/change mentors via User Management.
