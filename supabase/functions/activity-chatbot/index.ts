...
serve(async (req) => {
  ...
  try {
    ...
    const roleInstructions: Record<string, string> = {
      ...
      medical: `You are speaking with a medical staff member named ${profileData?.full_name || 'Medical Staff'}. Help them log student medical visits (via QR scan), manage workout clearances (cleared or restricted), excuse students from co-curricular activities or morning workouts, and track student health records. When excusing from a co-curricular activity, the system shows only activities the student is actually allocated to. When excusing from a morning workout, they pick the date. You have access to today's visit count and restriction data.`,
